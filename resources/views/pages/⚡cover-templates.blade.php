...
new class extends Component
{
    ...
    public function saveCover(string $html): void
    {
        ...
        if ($this->editingId) {
            ...
            if ($this->atLimit) {
                $this->addError('name', 'You can save up to '.CoverTemplate::MAX_PER_USER.' covers. Delete one to add another.');
        ...
        session()->flash('cover-tpl-saved', 'Saved “'.$this->name.'”.');
    }
...
<div class="min-h-screen bg-gray-100">
    <header class="border-b border-gray-200 bg-white">
        <div class="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div>
                <a href="{{ route('reports.index') }}" wire:navigate class="text-xs font-medium text-indigo-600 hover:text-indigo-500">&larr; Back to reports</a>
                <h1 class="text-base font-semibold text-gray-900">Cover Designer</h1>
            </div>
            <button type="button" wire:click="startNew" class="self-start rounded-md bg-white px-3 py-1.5 text-sm font-semibold text-gray-900 ring-1 ring-gray-300 hover:bg-gray-50 sm:self-auto">+ New cover</button>
        </div>
    </header>
    ...
    <div class="mx-auto flex max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:flex-row">
        ...
        >
            <div class="rounded-lg bg-white shadow-sm ring-1 ring-gray-200">
                ...
                <div class="flex flex-wrap items-center gap-2 border-b border-gray-200 px-4 py-2">
                    <input type="text" wire:model="name" placeholder="Name this cover (e.g. My TU Cover)" class="min-w-50 flex-1 rounded-md px-2 py-1.5 text-sm ring-1 ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                    <button type="button" x-on:click="$wire.saveCover($refs.canvas.innerHTML)" class="rounded-md bg-indigo-600 px-4 py-1.5 text-sm font-semibold text-white hover:bg-indigo-500">
                        {{ $editingId ? 'Update cover' : 'Save cover' }}
                    </button>
                </div>
                ...
                <div class="se-toolbar">
                    <button type="button" x-on:mousedown.prevent x-on:click="block('h1')" class="toolbar-btn font-semibold" title="Big title">Title</button>
                    <button type="button" x-on:mousedown.prevent x-on:click="block('h2')" class="toolbar-btn font-semibold" title="Subtitle">Subtitle</button>
                    <button type="button" x-on:mousedown.prevent x-on:click="block('p')" class="toolbar-btn" title="Normal text">Normal</button>
                    ...
                    <span class="toolbar-divider"></span>
                    <button type="button" x-on:mousedown.prevent x-on:click="cmd('justifyLeft')" class="toolbar-btn">Left</button>
                    <button type="button" x-on:mousedown.prevent x-on:click="cmd('justifyCenter')" class="toolbar-btn">Center</button>
                    <button type="button" x-on:mousedown.prevent x-on:click="cmd('justifyRight')" class="toolbar-btn">Right</button>
                    <span class="toolbar-divider"></span>
                    <label class="toolbar-btn cursor-pointer" title="Insert an image">
                        + Image
                        <input type="file" wire:model="image" accept="image/*" class="hidden">
                    </label>
                    <span wire:loading wire:target="image" class="se-group-label">Uploading…</span>
                </div>

                {{-- Canvas --}}
                <p class="px-4 pt-3 text-xs text-gray-500">Click into the page below and type. Select text, then use the buttons above. Add images with <strong>+ Image</strong>.</p>
                <div class="overflow-x-auto px-4 py-4">
                    <div wire:ignore x-ref="canvas" contenteditable="true" spellcheck="false"
                        class="cover-sheet-custom cover-custom mx-auto shadow ring-1 ring-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-400">
                    </div>
                </div>
            </div>
        </main>
        ...
        <aside class="w-full space-y-3 lg:w-72 lg:shrink-0">
            <div class="rounded-lg bg-white p-4 shadow-sm ring-1 ring-gray-200">
                <h2 class="text-xs font-semibold uppercase tracking-wide text-gray-500">Your saved covers</h2>
                <p class="mt-1 text-[11px] text-gray-400">Up to {{ \App\Models\CoverTemplate::MAX_PER_USER }}. Open one on a report from its cover page.</p>

                <ul class="mt-3 space-y-2">
                    @forelse ($this->templates as $template)
                        <li class="flex items-center justify-between gap-2 rounded-md px-2 py-1.5 text-sm {{ $editingId === $template->id ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700 hover:bg-gray-50' }}">
                            <button type="button" wire:click="edit({{ $template->id }})" class="flex-1 truncate text-left font-medium">{{ $template->name }}</button>
                            <button type="button" wire:click="delete({{ $template->id }})" wire:confirm="Delete this saved cover?" class="text-xs text-red-600 hover:text-red-800">Delete</button>
                        </li>
                    @empty
                        <li class="px-2 py-1.5 text-xs text-gray-500">No saved covers yet — design one and click Save.</li>
                    @endforelse
                </ul>

                @if ($this->atLimit && ! $editingId)
                    <p class="mt-3 rounded-md bg-amber-50 px-2 py-1.5 text-[11px] text-amber-800">You've saved {{ \App\Models\CoverTemplate::MAX_PER_USER }} covers. Delete one to create another.</p>
                @endif
            </div>
        </aside>
    </div>
