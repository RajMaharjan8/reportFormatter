    ...
    <div class="w-full max-w-md rounded-lg bg-white p-8 shadow-sm ring-1 ring-gray-200">
        <div class="text-center">
            ...
            <p class="mt-2 text-sm text-gray-600">
                {{ $step === 1 ? "Enter your email and we'll send a reset code." : "Enter the code we emailed and choose a new password." }}
            </p>
        </div>
        ...
        @if ($step === 1)
            <form wire:submit="sendCode" class="mt-8 space-y-4">
                ...
                <button type="submit" class="w-full rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
                    <span wire:loading.remove wire:target="sendCode">Send reset code</span>
        ...
        @else
            <form wire:submit="resetPassword" class="mt-8 space-y-4">
                <div>
                    ...
                    <input type="text" id="code" inputmode="numeric" maxlength="6" wire:model="code" autocomplete="one-time-code" class="mt-1 block w-full rounded-md px-3 py-2 text-center text-lg tracking-[0.5em] ring-1 ring-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500">
                ...
                <p class="text-center text-sm text-gray-600">
                    <button type="button" wire:click="sendCode" class="font-medium text-indigo-600 hover:text-indigo-500">Resend code</button>
                </p>
            </form>
        @endif

        <p class="mt-6 text-center text-sm text-gray-600">
            <a href="{{ route('login') }}" wire:navigate class="font-medium text-indigo-600 hover:text-indigo-500">Back to sign in</a>
        </p>
    </div>
</div>
