...
<div class="flex min-h-screen items-center justify-center bg-gray-50 px-4 py-12">
    ...
    <div class="w-full max-w-md rounded-lg bg-white p-8 shadow-sm ring-1 ring-gray-200">
        <div class="text-center">
            <h1 class="text-2xl font-semibold text-gray-900">Sign in to {{ config('app.name') }}</h1>
            <p class="mt-2 text-sm text-gray-600">Use your email and password, or continue with Google.</p>
        </div>

        @if (session('status'))
            <div class="mt-6 rounded-md bg-green-50 px-4 py-3 text-sm font-medium text-green-800 ring-1 ring-green-200">{{ session('status') }}</div>
        @endif
        @if (session('auth-error') || session('suspended'))
            <div class="mt-6 rounded-md bg-red-50 px-4 py-3 text-sm font-medium text-red-800 ring-1 ring-red-200">{{ session('auth-error') ?? session('suspended') }}</div>
        @endif
        ...
        <form wire:submit="authenticate" class="mt-8 space-y-4">
            ...
            <div>
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('forgot-password') }}" wire:navigate class="text-xs font-medium text-indigo-600 hover:text-indigo-500">Forgot password?</a>
                </div>
        ...
        <a href="{{ route('auth.google.redirect') }}"
           class="inline-flex w-full items-center justify-center gap-3 rounded-md bg-white px-4 py-2.5 text-sm font-semibold text-gray-800 ring-1 ring-gray-300 hover:bg-gray-50">
        ...
        <p class="mt-6 text-center text-sm text-gray-600">
            New here? <a href="{{ route('register') }}" wire:navigate class="font-medium text-indigo-600 hover:text-indigo-500">Create an account</a>
        </p>
    </div>
</div>
